...
import unitNumberFormat from "../../../../assets/unit_number_format.png";
...
<Aside type="note" title="Before You Begin">
  Ensure you have read and understood all **server rules**, **penal code**, and
  **communication procedures** before going on duty.
</Aside>
...
<Aside type="tip" title="Stay Updated">
  Server rules and policies may change. Review the documentation regularly to
  stay current.
</Aside>
...
2. **Vehicle Use**  
   You may use any vehicle and lighting setup provided it remains realistic and appropriate.

   - Any **marked vehicle** may be used.
   - **Visor lights may not be used** on vehicles equipped with a **full light bar**.
   - **Grille lights may not be used** on vehicles equipped with **ram bar lights**.
   - **Unmarked vehicles may be used**, provided there is **not an excessive number of unmarked units** already in service.

3. **LEO Vehicle Performance Restrictions**

    All **Law Enforcement vehicles** must remain **stock and unmodified** to ensure fair,
    balanced, and realistic roleplay.

    - **NO engine upgrades** (Engine Levels 1–4)
    ...
    These upgrades may **NOT** be applied through **vMenu**, the **F9 menu**, or any
    other in-game customization menu.

    LEO vehicles must be used **as provided by the server**.
...
5. **Suspect Processing**  
   You are not required to transport a suspect to the station unless it is necessary for roleplay or additional processing (e.g., fingerprints or mugshots).

   When possible, **ask the individual being arrested** whether they would prefer **county transport** or for **you to transport them**.
...
<Aside type="caution" title="Equipment Discipline">
  Unauthorized or excessive use of weapons, vehicles, or equipment may result in
  disciplinary action or removal of your LEO tag.
</Aside>

<Aside type="note" title="Allowed Modifications">
  Visual-only modifications are permitted as long as they do not affect vehicle
  performance.
</Aside>
...
<Aside type="note" title="Conflict Resolution">
  Always attempt to resolve situations through communication first. Escalation
  should occur only when verbal compliance fails or safety becomes a concern.
</Aside>
...
<Aside type="note" title="Signal Codes">
  **Signal 100** – Hold all but emergency traffic **Signal 60** – Drugs **Signal
  11** – Running radar **Code Zero** – Game crash **Code 4** – Under control
  **Code 5** – Felony stop or high-risk stop
</Aside>

<Aside type="caution" title="Miscommunication Risk">
  Incorrect or unclear radio communication can result in confusion, delays, or
  unsafe outcomes during active incidents.
</Aside>
...
<Aside type="caution" title="Key Principles">
  Always attempt to de-escalate when possible. Provide clear warnings before
  using force whenever it is safe to do so. Lethal force should always be the
  final option.
</Aside>
...
<Aside type="tip" title="Code 1 / 2 / 3 Usage">
  **Code 1** – Routine response, obey all traffic laws. **Code 2** – Urgent
  response without lights or sirens unless needed. **Code 3** – Emergency
  response with lights and sirens active.
</Aside>
...
<Aside type="tip" title="Traffic Stop Flow">
  1. Call out **10-11** with plate, location, occupant count, and reason for
  stop. 2. Position your vehicle for safety and cover. 3. Approach carefully,
  greet the driver, and explain the reason for the stop. 4. Run **10-27**,
  **10-28**, and **10-29** as needed. 5. Conclude by issuing a warning,
  citation, or arrest, then call **Code 4** when the scene is clear.
</Aside>

<Aside type="danger" title="Pursuit Protocol (10-80)">
  Primary unit calls direction of travel, speed, vehicle description, and
  traffic conditions. Request air support or spike strips as allowed. Supervisor
  approval (**10-71**) is required to continue a pursuit. Terminate when the
  risk to public safety outweighs the need for apprehension. Announce **Code 4**
...
<Aside type="caution" title="Zero Tolerance">
  Corruption, abuse of authority, and repeated rule violations are not tolerated
  and may lead to permanent removal from law enforcement roles.
</Aside>
...
<figure>
  <img
    src={unitNumberFormat.src}
    alt="Example unit number format used in Sonoran CAD"
    width={unitNumberFormat.width}
    height={unitNumberFormat.height}
  />
</figure>
...
<Aside type="note" title="Acknowledgment Statement">
  By serving as a Law Enforcement Officer in Denali State Roleplay, you agree to
  uphold these standards and understand that any violations can result in
  disciplinary action.
</Aside>
